...
ui_config_dialog, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'config_dialog_base.ui'), resource_suffix='')
...
class ConfigDialog(QDialog, ui_config_dialog):
    ...
    def readConfiguration(self):
        ...
        for section in config.sections():
            if section == 'xgApps':
                try:
                    self.txt_xgApps_local.setPlainText(config[section]['local_folder'])
                except:
                    pass
                try:
                    self.txt_xgApps_network.setPlainText(config[section]['network_folder'])
                except:
                    pass
                try:
                    showResults = config[section]['show_results']
                    if showResults == "yes":
                    ...
                    self.chk_show_results.setChecked(True)
            if section == 'dbConfig':
                index = self.cbo_db_type.findText(config[section]['db_type'], Qt.MatchFixedString)
                if index >= 0:
                    self.cbo_db_type.setCurrentIndex(index)

                self.txt_host.setPlainText(config[section]['host'])
                self.txt_port.setPlainText(config[section]['port'])
                self.txt_db.setPlainText(config[section]['database'])
                ...
                if trusted == "yes":
                    ...
                    if config[section]['db_type'] == u'PostGIS':
                        self.chk_trusted.setEnabled(False)
                    self.txt_user.setPlainText(config[section]['user'])
                    self.txt_pwd.setPlainText(config[section]['password'])
                createTable = config[section]['new_table']
                if createTable == "yes":
                    ...
                    self.txt_table.setPlainText(config[section]['table'])
                    self.txt_geom.setPlainText(config[section]['geom_col'])
                dbConfigRead = True
            # end if
        # next
    ...
    def saveConfiguration(self):
        ...
        try:
            ...
            QMessageBox.critical(self.iface.mainWindow(), 'Save Failed', 'Failed to write the configuration to {}: {}'.format(configFilePath, e))
